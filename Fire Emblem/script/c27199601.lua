...
function s.base(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:GetSummonType()==SUMMON_TYPE_SYNCHRO then
		...
		if Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then
			local tc=Duel.CreateToken(tp,27240000)
			Duel.Equip(tp,tc,c)
		end
	end
	--promotional gains
...
function s.astracon(e)
	local c=e:GetHandler()
	return Duel.GetAttacker()==c and c:GetFlagEffect(0xade972)==0 and c:GetFlagEffect(0x84a7e)==0
end
...
function s.atop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	c:ResetFlagEffect(0xade972)
	Duel.ChainAttack()
end

function s.adeptop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	c:RegisterFlagEffect(0xade97,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
	c:ResetFlagEffect(0xade972)
	Duel.ChainAttack()
end
...
function s.rctop(e,tp,eg,ep,ev,re,r,rp)
	...
	--adept
	if res1>=50-c:GetCounter(SKILL_COUNTER) and c:GetFlagEffect(0xade97)==0 then
		c:RegisterFlagEffect(0xade972,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
		...
		if res1>=50-c:GetCounter(SP_COUNTER) and c:GetFlagEffect(0xfe2a7)==0 then
			...
			c:ResetFlagEffect(0xade97)
		else
			--astra
			if res2>=85-c:GetCounter(SKILL_COUNTER) and c:GetFlagEffect(0xa574a)==0 then
				c:ResetFlagEffect(0xade97)
				local e1=Effect.CreateEffect(c)
				e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
				...
				e1:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
				e1:SetCountLimit(4)
				e1:SetCondition(s.astracon)
				e1:SetOperation(s.atop)
				c:RegisterEffect(e1)
				c:RegisterFlagEffect(0xa574a,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
			end
		end
	end
	--crit
