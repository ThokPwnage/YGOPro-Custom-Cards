...
function s.initial_effect(c)
	...
	c:EnableCounterPermit(0x64c)
	c:EnableCounterPermit(0xfe7)
	c:EnableReviveLimit()
	...
	e15:SetCondition(s.staffxpcon)
	e15:SetOperation(s.staffxpop)
	...
	e16:SetTarget(aux.TRUE)
	e16:SetValue(200)
	c:RegisterEffect(e16)
end

c27205600.lvupcount=nil
c27205600.lvup={}
...
function s.base(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:GetSummonType()==SUMMON_TYPE_SYNCHRO then
		...
		if Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then
			local tc=Duel.CreateToken(tp,27240700)
			Duel.Equip(tp,tc,c)
		end
	end
	--promotional gains
...
function s.coronacon(e,tp,re,r)
	local c=e:GetHandler()
	local ph=Duel.GetCurrentPhase()
	return c:GetFlagEffect(0xc0404a)~=0 and (ph==PHASE_DAMAGE_CAL or ph==PHASE_DAMAGE or ph==PHASE_BATTLE_START or ph==PHASE_BATTLE_STEP) and Duel.GetAttackTarget() and Duel.GetAttacker()==c
end

function s.coronatg(e,c)
	local bc=c:GetBattleTarget()
	return bc==e:GetHandler() and bc==Duel.GetAttacker()
end

function s.coronavala(e,tp,re,r)
	return Duel.GetAttackTarget():GetAttack()*(-0.5)
end

function s.coronavald(e,tp,re,r)
	return Duel.GetAttackTarget():GetDefense()*(-0.5)
end

function s.coronaop(e,tp,re,r)
	local c=e:GetHandler()
	c:ResetFlagEffect(0xc0404a)
end

function s.staffxpcon(e,tp,re,r)
	local c=e:GetHandler()
	return c:GetFlagEffect(0xfe64ca)~=0
end

function s.staffxpop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	s.lvop(e,tp,eg,ep,ev,re,r,rp)
	c:ResetFlagEffect(0xfe64ca)
end
...
function s.rctop(e,tp,eg,ep,ev,re,r,rp)
	...
	if res2>=85-c:GetCounter(SKILL_COUNTER) then
		c:RegisterFlagEffect(0xc0404a,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
		local e1=Effect.CreateEffect(c)
		...
		e1:SetCondition(s.coronacon)
		e1:SetTarget(s.coronatg)
		e1:SetValue(s.coronavala)
		e1:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
		c:RegisterEffect(e1)
		local e2=e1:Clone()
		e2:SetValue(s.coronavald)
		e2:SetCode(EFFECT_UPDATE_DEFENSE)
		c:RegisterEffect(e2)
		local e3=Effect.CreateEffect(c)
		e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
		e3:SetCode(EVENT_BATTLE_DAMAGE)
		e3:SetCondition(s.coronacon)
		e3:SetOperation(s.coronaop)
	...
	if Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED+LOCATION_EXTRA,0,1,nil,class,e,tp) then
		...
		if tc then
			...
			if staff then
				...
				local eq=Duel.CreateToken(tp,staff)
				Duel.Equip(tp,eq,tc)
				eq:AddCounter(DUR_COUNTER,StaffDur)
			end
