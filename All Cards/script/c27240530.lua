...
function s.initop(e,tp,re,r)
	...
	if tc:GetFlagEffect(0xfee92)==0 then 
		...
		tc:AddCounter(SP_COUNTER,5)
		tc:RegisterFlagEffect(0xfee97c,RESET_EVENT,0,1,0,0)
		tc:RegisterFlagEffect(0xfee959,RESET_EVENT,0,1,5,1)
...
function s.rmop(e,tp,re,r)
	local c=e:GetHandler()
	local tc=c:GetEquipTarget()
	tc:RemoveCounter(tp,SP_COUNTER,5,REASON_EFFECT)
end
...
function s.destop(e,tp,re,r)
	local c=e:GetHandler()
	local tc=c:GetEquipTarget()
	tc:RemoveCounter(tp,LCK_COUNTER,3,REASON_EFFECT)
	Duel.SendtoGrave(c,REASON_RULE)
end
