...
c27200300.lvupcount=1
c27200300.lvup={27200600}
...
function s.indesfilter(e,tp,re,r)
	if not (Duel.GetCurrentPhase()==PHASE_DAMAGE or Duel.GetCurrentPhase()==PHASE_DAMAGE_CAL) or not Duel.GetAttackTarget() then return end
	local c=e:GetHandler()
	local tc
	local dmg=0
	if Duel.GetAttacker()==c then
		tc=Duel.GetAttackTarget()
		if tc:IsSetCard(0x64a) or tc:IsPosition(POS_DEFENSE) then
			dmg=tc:GetDefense()-c:GetAttack()
		else
			dmg=tc:GetAttack()-c:GetAttack()
		end
	else
		tc=Duel.GetAttacker()
		if tc:IsPosition(POS_DEFENSE) then
			dmg=tc:GetDefense()-c:GetDefense()
		else
			dmg=tc:GetAttack()-c:GetDefense()
		end
	end
	return c:GetCounter(HP_COUNTER)>dmg/100
end
