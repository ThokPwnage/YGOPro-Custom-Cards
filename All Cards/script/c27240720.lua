...
function s.healcon(e,tp,re,r)
	local c=e:GetHandler()
	local tc=c:GetEquipTarget()
	return Duel.IsExistingMatchingCard(s.healfilter,tp,LOCATION_MZONE,0,1,tc)
end
...
function s.healop(e,tp,re,r)
	...
	if s.healcon(e,tp,re,r) then op=Duel.SelectOption(tp,aux.Stringid(id,0),aux.Stringid(id,1)) end
	if op==1 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
		local htg=Duel.SelectMatchingCard(tp,s.healfilter,tp,LOCATION_MZONE,0,1,1,tc):GetFirst()
...
function s.recop(e,tp,re,r)
	local c=e:GetHandler()
	local tc=c:GetEquipTarget()
	if not tc:AddCounter(HP_COUNTER,25) then
