...
function s.initial_effect(c)
	...
	e16:SetCondition(s.degamblecon)
	e16:SetOperation(s.degambleop)
	c:RegisterEffect(e16)
end

c27201600.lvupcount=nil
c27201600.lvup={}
...
function s.base(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:GetSummonType()==SUMMON_TYPE_SYNCHRO then
		...
		if Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then
			local tc=Duel.CreateToken(tp,27240200)
			Duel.Equip(tp,tc,c)
		end
	end
	--promotional gains
...
function s.degamblecon(e,tp,re,r)
	local c=e:GetHandler()
	return c:GetFlagEffect(0x9a337e)~=0 and c:GetFlagEffect(0x9a337e2)==0
end

function s.gambleop(e,tp,re,r)
	local c=e:GetHandler()
	c:RegisterFlagEffect(0x9a337e,RESET_EVENT,0,1)
	c:SetFlagEffectLabel(0x9a337e,c:GetCounter(LCK_COUNTER))	
	c:RegisterFlagEffect(0x9a337e59,RESET_EVENT,0,1)
	c:SetFlagEffectLabel(0x9a337e59,c:GetCounter(SP_COUNTER)/2)
	c:RegisterFlagEffect(0x9a337e54,RESET_EVENT,0,1)
	c:SetFlagEffectLabel(0x9a337e54,c:GetCounter(SKILL_COUNTER)/2)
	c:RegisterFlagEffect(0x9a337e2,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,2)
	c:AddCounter(LCK_COUNTER,c:GetCounter(LCK_COUNTER))
	c:RemoveCounter(tp,SP_COUNTER,c:GetCounter(SP_COUNTER)/2,REASON_EFFECT)
	c:RemoveCounter(tp,SKILL_COUNTER,c:GetCounter(SKILL_COUNTER)/2,REASON_EFFECT)
end

function s.degambleop(e,tp,re,r)
	local c=e:GetHandler()
	c:RemoveCounter(tp,LCK_COUNTER,c:GetFlagEffectLabel(0x9a337e),REASON_EFFECT)
	c:AddCounter(SP_COUNTER,c:GetFlagEffectLabel(0x9a337e59))
	c:AddCounter(SKILL_COUNTER,c:GetFlagEffectLabel(0x9a337e54))
	c:ResetFlagEffect(0x9a337e)
	c:ResetFlagEffect(0x9a337e59)
	c:ResetFlagEffect(0x9a337e54)
end
...
function s.colossuscon(e,tp,re,r)
	return e:GetHandler():GetFlagEffect(0xc0705505)~=0
end
...
function s.colossusval(e,tp,re,r)
...
function s.rctop(e,tp,eg,ep,ev,re,r,rp)
	...
	--colossus
	if res2>=85-c:GetCounter(SKILL_COUNTER) then
		c:RegisterFlagEffect(0xc0705505,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE)
		e1:SetProperty(EFFECT_FLAG_SINGLE_RANGE)
		e1:SetCode(EFFECT_UPDATE_ATTACK)
		e1:SetRange(LOCATION_MZONE)
		e1:SetCondition(s.colossuscon)
		e1:SetValue(s.colossusval)
		e1:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
		c:RegisterEffect(e1)
	end
end
