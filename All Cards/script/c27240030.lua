...
function s.initop(e,tp,re,r)
	...
	if tc:GetFlagEffect(0xfee92)==0 then 
		...
		tc:AddCounter(SP_COUNTER,1)
		tc:AddCounter(LCK_COUNTER,20)
		tc:AddCounter(SKILL_COUNTER,1)
		tc:RegisterFlagEffect(0xfee97c,RESET_EVENT,0,1,20,1)
		tc:RegisterFlagEffect(0xfee959,RESET_EVENT,0,1,1,1)
		tc:RegisterFlagEffect(0xfee954,RESET_EVENT,0,1,1,1)
	end
	tc:ResetFlagEffect(0xfee92)
	tc:ResetFlagEffect(0xfe42)
...
function s.disop1(e,tp,re,r)
	...
	if not c:RemoveCounter(tp,DUR_COUNTER,1,REASON_EFFECT) then 
		c:GetEquipTarget():RemoveCounter(tp,SP_COUNTER,1,REASON_EFFECT)
		c:GetEquipTarget():RemoveCounter(tp,SKILL_COUNTER,1,REASON_EFFECT)
...
function s.destcon(e,tp,re,r)
	local c=e:GetHandler()
	return c:GetCounter(DUR_COUNTER)==0 and c:GetEquipTarget() and c:GetFlagEffect(0xfee91)~=0
end

function s.destop(e,tp,re,r)
	local c=e:GetHandler()
	local tc=c:GetEquipTarget()
	tc:RemoveCounter(tp,SP_COUNTER,1,REASON_EFFECT)
	tc:RemoveCounter(tp,SKILL_COUNTER,1,REASON_EFFECT)
	tc:RemoveCounter(tp,LCK_COUNTER,20,REASON_EFFECT)
	Duel.SendtoGrave(c,REASON_RULE)
end
