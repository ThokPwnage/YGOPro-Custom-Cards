...
function s.initial_effect(c)
	...
	e15:SetValue(s.shadecon)
	c:RegisterEffect(e15)   
end

c27204600.lvupcount=nil
c27204600.lvup={}
...
function s.base(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:GetSummonType()==SUMMON_TYPE_SYNCHRO then
		...
		if Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then
			local tc=Duel.CreateToken(tp,27240400)
			Duel.Equip(tp,tc,c)
		end
	end
	--promotional gains
	...
	c:RegisterFlagEffect(0xfe42,RESET_EVENT,0,1)
	c:ResetFlagEffect(0xfec17)
end

function s.flarecon(e,tp,re,r)
	local c=e:GetHandler()
	local ph=Duel.GetCurrentPhase()
	return c:GetFlagEffect(0xf7a4e)~=0 and (ph==PHASE_DAMAGE_CAL or ph==PHASE_DAMAGE or ph==PHASE_BATTLE_START or ph==PHASE_BATTLE_STEP) and Duel.GetAttackTarget() and Duel.GetAttacker()==c
end

function s.flareop(e,tp,re,r)
	local c=e:GetHandler()
	local dmg=Duel.GetBattleDamage(1-tp)
	if dmg and not c:AddCounter(HP_COUNTER,dmg/200) then
		c:AddCounter(HP_COUNTER,c:GetCounter(MAXHP_COUNTER)-c:GetCounter(HP_COUNTER))
	end
	c:ResetFlagEffect(0xf7a4e)
end

function s.shadefilter(c)
	return not c:IsHasEffect(EFFECT_CANNOT_BE_BATTLE_TARGET) and not c:IsHasEffect(EFFECT_IGNORE_BATTLE_TARGET)
end

function s.shadecon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return Duel.IsExistingMatchingCard(s.shadefilter,c:GetControler(),LOCATION_MZONE,0,1,c)
end
...
function s.rctop(e,tp,eg,ep,ev,re,r,rp)
	...
	if res2>=85-c:GetCounter(SKILL_COUNTER) then
		c:RegisterFlagEffect(0xf7a4e,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE)
		e1:SetCode(EFFECT_SET_ATTACK_FINAL)
		e1:SetRange(LOCATION_MZONE)
		e1:SetProperty(EFFECT_FLAG_IGNORE_IMMUNE)
		e1:SetCondition(s.flarecon)
		...
		e3:SetCondition(s.flarecon)
		e3:SetOperation(s.flareop)
